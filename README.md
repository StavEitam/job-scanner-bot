...
- **Dynamic search, not hardcoded.** `/scan Product Manager` searches only that role; a bare `/scan` falls back to a default watchlist of roles. The AI filter's matching criteria are built from whatever was actually requested that run — not a fixed list baked into the prompt.
...
- **Two-tier filtering, not one.** A free regex pre-filter rejects postings with unambiguous seniority signals in the *title* (Senior/Director/VP/Head of, Hebrew equivalents too) before any AI call happens. It deliberately does **not** reject plain "Manager" — that would false-positive on "Product Manager," one of the actual target roles. Everything else (experience stated only in the body text, nuanced title matching) is left to the LLM, which is the part that actually needs judgment.
...
- **AllJobs.co.il and JobMaster.co.il** — no ready-made Apify actor exists for either (verified by searching the store). Would require a custom scraper.
- **Comeet-hosted company career pages** — covered by a dedicated actor (`automation-lab/comeet-jobs-scraper`), but it scrapes one company career page at a time rather than searching broadly, so it needs a maintained list of target companies. Not yet wired in.
- **Telegram/WhatsApp job-posting groups** — no ready tool; would need a custom listener (e.g. Telethon for Telegram) feeding into the same AI filter node.
- **UX is still developer-facing.** No `/help`, no way to persist a custom role list without retyping it every time, no inline feedback (👍/👎) to teach the filter over time, no alerting if a scraper silently fails.
- **No spend guard.** Nothing currently caps Apify/Claude usage per day — worth adding given a mid-build session where Apify credits ran out unexpectedly.
...
4. Create a Google Sheet with columns: `job_id`, `title`, `company`, `link`, `date_sent`.
5. Message your bot `/scan` or `/scan <role1, role2, ...>`.

## Status

Working end-to-end, including the batching/chunking rework: a live `/scan` run against the default 5-role watchlist pulled ~485 raw postings across all 4 sources, ~353 survived the free keyword pre-filter, deduplication cut that further, and the chunked AI pass (35 postings per Claude call) correctly evaluated all of them — no truncation, no silently dropped jobs — before dedup-against-history and delivery.

Not yet done: Comeet integration, AllJobs/JobMaster custom scraping, `/help` + persisted role list + 👍/👎 feedback loop, cross-source fuzzy dedup (same real posting appearing on two different boards with different `job_id`s), and a usage/cost guard.
